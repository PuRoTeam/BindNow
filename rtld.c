...
# endif
  ...
  /* Call the OS-dependent function to set up life so we can do things like
     ...
     entry point on the saexec_nameme stack we entered on.  */
...
#define BIND_NOW_SECTION ".bind_now" 
static void find_now_section()
{
  const char *exec_name = "/proc/self/exe"; //current executable path
  int fd = open(exec_name, O_RDONLY);
  
  struct stat file_stats;
  if(stat(exec_name, &file_stats) != 0) //get file status. We need it to get file size
    return;

  char *mapped_file = mmap(0, file_stats.st_size, PROT_READ, MAP_PRIVATE, fd, 0); //mapping file to memory
  
  ElfW(Ehdr) *rtld_ehdr = (ElfW(Ehdr)*)mapped_file; //ELF header
  ElfW(Shdr) *rtld_shdr = (ElfW(Shdr)*)(mapped_file + rtld_ehdr->e_shoff); //Section Header Table
  	 
  ElfW(Word) shnum = rtld_ehdr->e_shnum; //number of entry of Section Header Table
  
  if(rtld_ehdr->e_shstrndx != SHN_UNDEF) { //Check if Section Name String Table Header is defined
    ElfW(Shdr)* sh_strtbl = rtld_shdr + rtld_ehdr->e_shstrndx; //Section Name String Table Header
   
    if(sh_strtbl->sh_size > 0) { //check that Section Header String Table size is greater than zero
      ElfW(Shdr)* sh; //current Section Header
	
      for(sh = rtld_shdr; sh < &rtld_shdr[shnum]; ++sh) {
        char* strtbl = mapped_file + sh_strtbl->sh_offset; //Section Name String Table
        char* section_name = strtbl + sh->sh_name; //name of current section

        if(strcmp(section_name, BIND_NOW_SECTION) == 0) { //is this ".bind_now" section?
          GLRO(dl_lazy) = 0; //disabling lazy binding
          break;
        }
      }
    }
